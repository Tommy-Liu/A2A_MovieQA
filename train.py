...
class TrainManager(object):
    def __init__(self):
        ...
        self.loss = tf.losses.softmax_cross_entropy(tf.one_hot(self.train_data.gt, 5), self.train_model.output)
        ...
        self.val_accuracy, self.val_accuracy_update = tf.metrics.accuracy(self.val_data.gt,
                                                                          tf.argmax(self.val_model.output, axis=0),
                                                                          name='val_accuracy')
            ...
            try:
                while True:
                    ...
                    with trange(len(self.val_data)) as pbar:
                        ...
                        sw.add_summary(summary, step)
                    if acc > max_acc:
                        self.saver.save(sess, self._best_checkpoint, step)
