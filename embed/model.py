...
class NGramModel(object):
    def __init__(self, data, oracle):
        ...
        self.embedding_matrix = tf.get_variable("embedding_matrix", [self.data.vocab_size - 1, 300],
                                                tf.float32, self.initializer, trainable=True)
        zero_vec = tf.get_variable("zero_vec", [1, 300], tf.float32, tf.constant_initializer(0),
                                   trainable=False)
        self.gram_matrix = tf.concat([self.embedding_matrix, zero_vec], axis=0)

        self.gram_embedding = tf.nn.embedding_lookup(self.gram_matrix, self.data.word)
