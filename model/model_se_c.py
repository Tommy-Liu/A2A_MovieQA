...
class Model(object):
    def __init__(self, data, training=False):
        ...
        with tf.variable_scope('Embedding_Linear'):
            # (1, E_t)
            self.ques = l2_norm(tf.layers.dense(self.data.ques, hp['emb_dim'], use_bias=False))
            # (5, E_t)
            self.ans = l2_norm(tf.layers.dense(self.data.ans, hp['emb_dim'], use_bias=False, reuse=True))
            # (N, E_t)
            self.subt = l2_norm(tf.layers.dense(self.data.subt, hp['emb_dim'], use_bias=False, reuse=True))
        ...
        with tf.variable_scope('Abstract'):
            # (1, N_c, E_t)
            self.compact_subt = tf.layers.max_pooling1d(tf.expand_dims(self.subt, axis=0), 6, 4)
            # (1, N_c, 1)
            self.attn = tf.tensordot(self.compact_subt, self.ques, axes=[[2], [1]])
            ...
            # inner_state = tf.get_variable('inner_state', shape=[6, hp['emb_dim']], initializer=tf.zeros_initializer(),
            #                               trainable=False)
            # watch_block = tf.range(5, s_shape[0], 6, dtype=tf.int32)

            # start_mask = tf.sequence_mask(6, s_shape[0])
            # end_mask = tf.sequence_mask(0, s_shape[0])
            # mask = tf.logical_xor(start_mask, end_mask)
            #
            # self.clips = tf.boolean_mask(self.subt, mask)
            # (1, N_c, E_t)
            self.subt_abst = self.compact_subt * self.attn
