...
class Model(object):
    def __init__(self, data, beta=0.0, training=False):
        ...
        with tf.variable_scope('Abstract'):
            ...
            # # (1, N, E_t / 2)
            self.conv2 = tf.layers.conv1d(self.conv1, 1, 3, dilation_rate=2,
            ...
            # (N, 1, E_a)
            self.attn = tf.transpose(self.attn, [1, 0, 2])
            # (N, 5, E_a)
            self.response = tf.matmul(self.confuse, self.attn)
            # (5, E_a, N)
            ...
            self.top_k_output, _ = tf.nn.top_k(tf.squeeze(self.response), 2)
            # (1, 5)
