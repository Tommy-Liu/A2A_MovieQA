...
class Model(object):
    def __init__(self, data, training=False):
        ...
        with tf.variable_scope('Language_Encode'):
            self.ques_enc = l2_norm(variance_encode(self.ques_embedding, self.data.ql), axis=1)
            self.ans_enc = l2_norm(variance_encode(self.ans_embedding, self.data.al), axis=1)
            self.subt_enc = l2_norm(variance_encode(self.subt_embedding, self.data.sl), axis=1)
