...
_mp = MovieQAPath()
hp = {'emb_dim': 300, 'feat_dim': 512, 'dropout_rate': 0.4}
...
class Model(object):
    def __init__(self, data, scale=0.0, training=False):
        ...
        with tf.variable_scope('Embedding_Linear'):
            ...
            # (5, E_t)
            self.ans = tf.layers.dense(self.raw_ans, hp['emb_dim'],  # reuse=True,
        ...
        with tf.variable_scope('Response'):
            # (1, N_s, E_t)
            self.conv_subt = tf.layers.conv1d(tf.expand_dims(self.subt, 0), hp['emb_dim'], 30, 10, padding='same',
                                              kernel_initializer=init, kernel_regularizer=reg)
            self.conv_subt = l2_norm(self.conv_subt, 2)
            self.conv_subt = dropout(self.conv_subt, training)

            # (1, N_s, E_t)
            # self.conv_subt = l2_norm(self.conv_subt * tf.expand_dims(self.ques, 0), 2)
            # # (1, N_ss, E_t)
            # self.conv_subt = tf.layers.conv1d(self.conv_subt, hp['emb_dim'], 30, 10, padding='same',
            #                                   kernel_initializer=init, kernel_regularizer=reg)
            # self.conv_subt = l2_norm(self.conv_subt, 2)
            # self.conv_subt = dropout(self.conv_subt, training)
            # (N_ss, E_t)
            self.conv_subt = tf.squeeze(self.conv_subt, 0)

            self.sq = tf.matmul(self.conv_subt, self.ques, transpose_b=True)
            ...
            # (1, E_t)
            self.abs = tf.reduce_sum(self.attn * self.conv_subt, axis=0, keepdims=True)
            self.abs = l2_norm(self.abs)

            alpha = tf.nn.sigmoid(tf.get_variable('alpha', [], initializer=tf.zeros_initializer()))
            self.abs = l2_norm((1 - alpha) * self.abs + alpha * self.ques, 1)
