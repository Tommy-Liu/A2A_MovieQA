...
class Model(object):
    def __init__(self, data, training=False):
        ...
        with tf.variable_scope('Abstract'):
            # (1, N, E_t)
            self.conv1 = l2_norm(tf.layers.conv1d(tf.expand_dims(self.subt, 0), hp['emb_dim'], 30,
                                                  activation=tf.nn.relu, padding='same', kernel_regularizer=reg), 2)
            # (1, N, E_t / 2)
            self.conv2 = l2_norm(tf.layers.conv1d(self.conv1, hp['emb_dim'] / 4, 30, dilation_rate=2,
                                                  activation=tf.nn.relu, padding='same', kernel_regularizer=reg), 2)
            # (1, N, E_t / 4)
            self.conv3 = l2_norm(tf.layers.conv1d(self.conv2, hp['emb_dim'] / 8, 30, dilation_rate=3,
                                                  activation=tf.nn.relu, padding='same', kernel_regularizer=reg), 2)
            # (1, N, E_t / 8)
            self.conv4 = l2_norm(tf.layers.conv1d(self.conv3, hp['emb_dim'] / 16, 30, dilation_rate=4,
                                                  activation=tf.nn.relu, padding='same', kernel_regularizer=reg), 2)
            # (N, E_t, E_t / 32)
