...
def create_one_tfrecord(qa, args, video_data, shard_id):
    ...
    with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
        for idx in range(start_ndx, end_ndx):
            ins = qa[idx]
            qa_embedding = np.load(join(_mp.encode_dir, ins['qid'] + '.npy'))
            ques = qa_embedding[0]
...
def main():
    ...
    encode_qa = du.json_load(_mp.tokenize_qa)
    ...
    if args.count:
        ...
        if 'test' in split:
            create_tfrecord(encode_qa, 'test', args.mode, args.num_per_shards)
